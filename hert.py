...
def main():
    ...
    with col_2:
        ...
        EKG=st.number_input('EKG',min_value=0,max_value=2)
    ...
    input_dict={'Exercise_angina':Exercise_angina,'FBS_over_120':FBS_over_120,'Age':Age,'Chest_pain_type':Chest_pain_type,'Sex':Sex,'EKG':EKG,'BP':BP,'Cholesterol':Cholesterol,'Max_HR':Max_HR,'ST_depression':ST_depression,'ST_slope':ST_slope,'vessels_fluro':vessels_fluro,'Thallium':Thallium}
    input_df=pd.DataFrame(input_dict,index=[0])

    Button=st.button('Predict')

    if Button:
        Heart_Disease= loaded_model.predict(input_df)
        if Heart_Disease==0:
        ...
        with precision:
            st.metric(label="Precision Score", value="86%")
        with recall:
            st.metric(label="Recall Score", value="86%")
    
        with f1:
            st.metric(label="F1 Score", value="86%")
        with acc:
            st.metric(label="Accuracy Score", value="86%")
